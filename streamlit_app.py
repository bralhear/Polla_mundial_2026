...
st.set_page_config(page_title="Mundialista 2026 - Polla", page_icon="⚽", layout="wide")
DB = "mundialista2026_simple.db"

PARTIDOS_INICIALES = [{'match_number': 1,
 ...
 {'match_number': 88,
  ...
  'ciudad': 'Sede por definir'},
 {'match_number': 89,
  'fecha_partido': '2026-07-5 18:00:00',
  ...
  'ciudad': 'Sede por definir'},
 {'match_number': 90,
  'fecha_partido': '2026-07-6 14:00:00',
  'stage': 'Octavos de Final',
  ...
  'ciudad': 'Sede por definir'},
 {'match_number': 91,
  'fecha_partido': '2026-07-7 18:00:00',
  'stage': 'Octavos de Final',
  ...
  'ciudad': 'Sede por definir'},
 {'match_number': 92,
  'fecha_partido': '2026-07-4 14:00:00',
  'stage': 'Octavos de Final',
 ...
 {'match_number': 97,
  ...
  'ciudad': 'Sede por definir'},
 {'match_number': 98,
  'fecha_partido': '2026-07-9 15:00:00',
  'stage': 'Cuartos de Final',
...
def init_db():
    ...
    if not cur.fetchone():
        cur.execute(
            ...
            ("Administrador", "admin@mundialista2026.com", hash_pw("1234"), datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        )
        conn.commit()
...
def seed_matches_if_empty():
    ...
    if total == 0:
        for m in PARTIDOS_INICIALES:
            cur.execute(
                "INSERT INTO matches (match_number, fecha_partido, stage, home_team, away_team, estadio, ciudad) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (m['match_number'], m['fecha_partido'], m['stage'], m['home_team'], m['away_team'], m['estadio'], m['ciudad'])
            )
        conn.commit()
...
def guardar_prediccion(user_id, match_id, home_pred, away_pred):
    if prediction_exists(user_id, match_id):
        raise ValueError("Ese partido ya lo registraste y quedó bloqueado.")
    cur.execute(
...
def get_user_predictions(user_id):
    return pd.read_sql_query("""
        SELECT p.id, m.match_number, m.fecha_partido, m.stage, m.home_team, m.away_team,
               m.home_score, m.away_score, p.home_pred, p.away_pred, p.puntos
        FROM predictions p
...
def actualizar_resultado_oficial(match_id, home_score, away_score):
    cur.execute("UPDATE matches SET home_score=?, away_score=? WHERE id=?", (int(home_score), int(away_score), match_id))
    conn.commit()

def get_predictions_by_match(match_id):
    return pd.read_sql_query("""
        SELECT p.id, u.nombre, p.home_pred, p.away_pred, p.puntos
        FROM predictions p
...
def set_points(prediction_id, puntos):
    cur.execute("UPDATE predictions SET puntos=? WHERE id=?", (int(puntos), prediction_id))
    conn.commit()

def get_ranking():
    return pd.read_sql_query("""
        SELECT u.nombre, COALESCE(SUM(p.puntos),0) AS puntos, COALESCE(COUNT(p.id),0) AS pronosticos
        FROM users u
...
if st.session_state.user is None:
    ...
    with tabs[0]:
        ...
        if len(df_matches) == 0:
            ...
            df_matches['label'] = df_matches.apply(lambda r: f"#{int(r['match_number'])} | {fmt_dt(r['fecha_partido'])} | {r['home_team']} vs {r['away_team']}", axis=1)
            ...
            if user['admin']:
                ...
                c1, c2 = st.columns(2)
                hs = c1.number_input(f"Goles oficiales {row['home_team']}", 0, 30, int(row['home_score']) if pd.notna(row['home_score']) else 0, 1, key='official_h')
                aw = c2.number_input(f"Goles oficiales {row['away_team']}", 0, 30, int(row['away_score']) if pd.notna(row['away_score']) else 0, 1, key='official_a')
                if st.button('Guardar resultado oficial', width='stretch', key='save_official'):
                    actualizar_resultado_oficial(int(row['id']), hs, aw)
                    st.success('✅ Resultado oficial guardado.')
                    st.rerun()

                st.markdown('---')
                st.write('### Poner puntos a los usuarios')
                    ...
                    for _, pr in preds.iterrows():
                        with st.container(border=True):
                            ...
                            with a:
                                ...
                                st.caption(f"Pronóstico: {int(pr['home_pred'])} - {int(pr['away_pred'])}")
                            with b:
                                puntos = st.selectbox('Puntos', [0,1,2], index=int(pr['puntos']) if pd.notna(pr['puntos']) else 0, key=f"pts_{int(pr['id'])}")
                            with c:
                                st.write('')
                                if st.button('Guardar', key=f"save_pts_{int(pr['id'])}", width='stretch'):
                                    set_points(int(pr['id']), int(puntos))
                ...
                if existing:
                    ...
                    c1, c2 = st.columns(2)
                    hp = c1.number_input(f"Goles {row['home_team']}", 0, 30, 0, 1, key='pred_h')
                    ap = c2.number_input(f"Goles {row['away_team']}", 0, 30, 0, 1, key='pred_a')
                    if st.button('Guardar mi pronóstico', width='stretch'):
                        try:
                ...
                if len(my_preds) > 0:
                    ...
                    st.dataframe(
                        disp[['match_number','Fecha','stage','home_team','away_team','Mi pronóstico','Resultado oficial','puntos']].rename(columns={
                            'match_number':'#', 'stage':'Fase', 'home_team':'Local', 'away_team':'Visitante', 'puntos':'Puntos'
                        }),
    ...
    with tabs[1]:
        ...
        if len(rank) > 0:
            ...
            st.dataframe(rank[['Posición','nombre','puntos','pronosticos']].rename(columns={'nombre':'Nombre','puntos':'Puntos','pronosticos':'Pronósticos'}), width='stretch', hide_index=True)
        else:
